...
public class CsvDataWriter {
    ...
    protected void writePredictionData(List<WaitTimeOutput> output) {
        ...
        try {
            ...
            CSVFormat csvFormat = CSVFormat.Builder.create(CSVFormat.EXCEL).setHeader(PredictionHeaders.class).build();

            csvPrinter = new CSVPrinter(
                    new FileWriter("src/resources/wait-time-output-actual.csv", StandardCharsets.UTF_8), csvFormat);

            int i = 0;
            while (i < output.size()) {
                csvPrinter.printRecord(output.get(i).getrideId(), output.get(i).getPeopleInLineCurrently(),
                        output.get(i).getpredictedWaitTime(), output.get(i).getStatusString());
                i++;
            }
